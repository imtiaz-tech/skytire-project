...
export default function BoltOnWireWheelForm({ editBoltOnWireWheelId, duplicateId }: BoltOnWireWheelFormProps) {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && !duplicateId) {
      const id = sessionStorage.getItem('duplicateBoltOnWireWheelId');
      if (id) {
        setActiveDuplicateId(id);
        sessionStorage.removeItem('duplicateBoltOnWireWheelId');
      }
    }
  }, [duplicateId]);
  ...
  const fetchBrandsData = async () => {
    try {
      const brandsRes = await axios.get('/api/admin/brands/dropdown?category=bolt_on_wheels');
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      const targetId = editBoltOnWireWheelId || activeDuplicateId;
      if (!targetId) return;

      setFetching(true);
      try {
        const res = await axios.get(`/api/admin/bolt-on-wire-wheels/${targetId}`);
    ...
    loadInitialData();
  }, [editBoltOnWireWheelId, activeDuplicateId]);
  ...
  const handleSubmit = async (e: React.FormEvent, statusOverride?: 'published' | 'draft') => {
    ...
    try {
      ...
      if (editBoltOnWireWheelId) {
        await dispatch(updateBoltOnWireWheel({ id: editBoltOnWireWheelId, data: submitData })).unwrap();
        toast.success('Bolt-On Wire Wheel updated successfully');
      } else {
        await dispatch(createBoltOnWireWheel(submitData)).unwrap();
        toast.success('Bolt-On Wire Wheel created successfully');
      }

      router.push('/admin/bolt-on-wire-wheels');
      router.refresh();
    } catch (err: any) {
  ...
  if (fetching) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-gray-400 font-medium animate-pulse">Loading bolt-on wire wheel data...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 py-8 animate-in fade-in duration-500 max-w-7xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-[#1e2a4a]">
          {editBoltOnWireWheelId ? 'Edit Bolt-On Wire Wheel' : 'Add New Bolt-On Wire Wheel'}
        </h1>
      </div>
      ...
      <form onSubmit={(e) => handleSubmit(e, 'published')} className="space-y-12">
        ...
        <div className="bg-white rounded-[32px] p-8 shadow-sm border border-gray-100 space-y-8">
          ...
          <div className="space-y-8 pt-2">
            ...
            {editBoltOnWireWheelId && sourceInventories.length > 0 && (
