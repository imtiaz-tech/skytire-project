...
import { ArrowLeft, Loader2, UploadCloud, X, Settings2, Check, ChevronDown, Calculator, Plus, GripVertical } from 'lucide-react';
    ...
    <div className="space-y-8 py-8 animate-in fade-in duration-500 max-w-7xl mx-auto">
      ...
      <form onSubmit={(e) => handleSubmit(e, 'published')} className="space-y-12">
            ...
            {productImages.map((item, idx) => {
              ...
              return (
                <div
                  ...
                  className={`w-[140px] h-[140px] relative rounded-[24px] overflow-hidden border group flex-shrink-0 bg-white flex items-center justify-center cursor-grab active:cursor-grabbing transition-all ${
                    isDragOver ? 'border-blue-500 ring-2 ring-blue-200 scale-[1.02]' : 'border-gray-100'
                  }`}
                  title="Drag to reorder"
                >
                  <img src={src} alt={`Product ${idx + 1}`} className="w-full h-full object-cover pointer-events-none" />
        ...
        <ProductCommonFields
          ...
          descriptionPlaceholder={editWheelId ? '' : 'Enter wheel description...'}
          seoCardTitle=""
          ...
          scoreFields={[
            { key: 'finishDurabilityScore', label: 'Finish Durability Score' },
            { key: 'impactResistanceScore', label: 'Impact Resistance Score' },
            { key: 'fitmentPrecisionScore', label: 'Fitment Precision Score' },
            { key: 'feedbackScore', label: 'Feedback Score' },
          ]}
          scores={{
            finishDurabilityScore: formData.finishDurabilityScore,
            impactResistanceScore: formData.impactResistanceScore,
            fitmentPrecisionScore: formData.fitmentPrecisionScore,
