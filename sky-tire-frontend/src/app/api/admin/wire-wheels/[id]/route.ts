...
async function getUniqueSlug(baseSlug: string, currentId?: string) {
  ...
  let counter = 1;
  while (true) {
    const existing = await prisma.wireWheel.findFirst({
      where: {
        slug,
...
export async function GET(
  ...
  try {
    ...
    const wireWheel = await prisma.wireWheel.findUnique({
      where: { id },
      include: {
        brand: true,
        source: true,
    ...
    if (!wireWheel) {
      return NextResponse.json({ error: 'Wire Wheel not found' }, { status: 404 });
    }

    const withInventory = await attachSourceInventories('wireWheel', wireWheel);
...
export async function PUT(
  ...
  try {
    ...
    const currentWireWheel = await prisma.wireWheel.findUnique({
      where: { id },
      include: { source: true },
    });
    ...
    const brandChanged = brandId !== currentWireWheel.brandId;
    const nameChanged = name !== currentWireWheel.name;
    const sizeChanged = size !== currentWireWheel.size;

    let slug = currentWireWheel.slug;
    let oldSlugs = [...currentWireWheel.oldSlugs];
    ...
    if (brandChanged || nameChanged || sizeChanged) {
      ...
      const baseSlug = generateSeoSlug(name || 'draft-wire-wheel', brandName, size);
      const uniqueSlug = await getUniqueSlug(baseSlug, id);
      if (uniqueSlug !== currentWireWheel.slug) {
        slug = uniqueSlug;
        if (!oldSlugs.includes(currentWireWheel.slug)) {
          oldSlugs.push(currentWireWheel.slug);
        }
      }
    }

    const updatedWireWheel = await prisma.wireWheel.update({
      where: { id },
      data: {
        sku: sku || null,
        alternatePartNumber: alternatePartNumber || null,
        upcNo: upcNo || null,
        name: name || 'Draft Wire Wheel',
    ...
    return NextResponse.json(updatedWireWheel);
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await prisma.wireWheel.delete({
      where: { id },
    });

    return NextResponse.json({ message: 'Wire Wheel deleted successfully' });
  } catch (error) {
