...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [wireWheels, total] = await Promise.all([
    ...
    console.error('Error fetching wire wheels:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch brand name for slug generation
    let brandName = '';
    if (brandId) {
    ...
    const newWireWheel = await prisma.wireWheel.create({
      data: {
    ...
    return NextResponse.json(newWireWheel);
  } catch (error) {
    console.error('Error creating wire wheel:', error);
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const result = await prisma.wireWheel.updateMany({
      where: { id: { in: ids } },
      data: { isActive },
    });
    ...
    console.error('Error bulk-updating wire wheels:', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
