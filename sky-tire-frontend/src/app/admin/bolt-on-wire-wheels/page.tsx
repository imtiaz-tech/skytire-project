...
import { fetchBoltOnWireWheels as fetchWireWheels, deleteBoltOnWireWheel as deleteWireWheel, bulkUpdateBoltOnWireWheels as bulkUpdateWireWheels } from '@/features/bolt-on-wire-wheels/slice';
import Pagination from '@/components/ui/Pagination';
import ConfirmModal from '@/components/common/ConfirmModal';
import { Pencil, Trash2 as Trash2Icon, Plus as PlusIcon, Copy as CopyIcon, Image as ImageIcon, Eye as EyeIcon } from 'lucide-react';
...
export default function BoltOnWireWheelsPage() {
  const router = useRouter();
  const dispatch = useAppDispatch();
  const { boltOnWireWheels: wireWheels, loading, total, pages, currentPage } = useAppSelector((state) => state.boltOnWireWheels);
  ...
  const confirmDelete = async () => {
    if (wireWheelToDelete) {
      try {
        ...
        toast.success('Bolt-On Wire Wheel deleted successfully');
      } catch (error: any) {
        toast.error(error.message || 'Failed to delete bolt-on wire wheel');
      } finally {
  ...
  const handleBulkAction = async () => {
    ...
    try {
      ...
      toast.success(
        `${selectedIds.size} bolt-on wire wheel${selectedIds.size > 1 ? 's' : ''} marked as ${targetIsActive ? 'Visible' : 'Invisible'}`
      );
      setSelectedIds(new Set());
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="flex items-center justify-between mt-15">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-[#1e2a4a]">Bolt-On Wire Wheels List</h1>
        </div>
        <Link
          href="/admin/bolt-on-wire-wheels/add"
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-4">
          ...
          <div className="flex items-center gap-8 flex-1">
            ...
            <div className="ml-auto text-[15px] font-bold text-gray-700 whitespace-nowrap">
              Total Bolt-On Wire Wheels: <span className="text-[#1e2a4a]">({total})</span>
            </div>
          </div>
        </div>
      </div>
                ...
                <tr>
                  <td colSpan={isBulkTab ? 12 : 11} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-gray-400 text-sm font-medium">Fetching bolt-on wire wheels...</p>
                    </div>
                  </td>
                </tr>
              ) : wireWheels.length === 0 ? (
                <tr>
                  <td colSpan={isBulkTab ? 12 : 11} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-gray-400 font-semibold">No bolt-on wire wheels found</p>
                    </div>
                  </td>
                </tr>
              ) : (
      ...
      <ConfirmModal
        ...
        title="Delete Bolt-On Wire Wheel"
        message="Are you sure you want to delete this bolt-on wire wheel? This action cannot be undone."
      ...
      <BoltOnWireWheelPreviewModal
        ...
        boltOnWireWheel={selectedWireWheel}
      />
    </div>
  );
}
