...
import { Search, Loader2, Plus, Edit2, Trash2, Database } from 'lucide-react';
...
export default function InventorySourcesPage() {
  ...
  const { inventorySources, loading, total, pages, currentPage } = useAppSelector((state) => state.inventorySources);
  ...
  const confirmDelete = async () => {
    if (sourceToDelete) {
      try {
        await dispatch(deleteInventorySource(sourceToDelete)).unwrap();
        toast.success('Inventory Source deleted successfully');
      } catch (error: any) {
        toast.error(error || 'Failed to delete inventory source');
      } finally {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="flex items-center justify-between mt-15">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-[#1e2a4a]">Inventory Sources</h1>
        </div>
        <Link
          href="/admin/inventory-sources/add"
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          ...
          <div className="ml-auto text-[15px] font-bold text-gray-700 whitespace-nowrap">
            Total Sources: <span className="text-[#1e2a4a]">({total})</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden animate-in fade-in duration-500">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-50">
                <th className="px-8 py-5 text-[14px] font-bold text-gray-400 uppercase tracking-[0.1em]">Source Name</th>
                ...
                <tr>
                  <td colSpan={2} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-gray-400 text-sm font-medium">Fetching sources...</p>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={2} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-gray-400 font-semibold">No inventory sources found</p>
                    </div>
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={source.id} className="hover:bg-gray-50/50 transition-all group">
                    ...
                    <td className="px-8 py-5 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-3">
                        <Link
                          href={`/admin/inventory-sources/edit/${source.id}`}
        ...
        {/* Pagination */}
        {!loading && inventorySources.length > 0 && (
