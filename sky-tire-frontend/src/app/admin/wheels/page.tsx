...
import {
  ...
  ArrowUp, ArrowDown, ArrowUpDown, Copy, Eye, EyeOff,
} from 'lucide-react';
import { useAppDispatch, useAppSelector } from '@/redux/hooks';
import { fetchWheels, deleteWheel, bulkUpdateWheels } from '@/redux/slices/wheelsSlice';
...
export default function WheelsPage() {
  ...
  useEffect(() => {
    dispatch(fetchWheels(buildParams()));
  ...
  // ── Checkbox helpers ──────────────────────────────────────────────────────
  const allChecked = wheels.length > 0 && wheels.every((w) => selectedIds.has(w.id));
  const someChecked = wheels.some((w) => selectedIds.has(w.id)) && !allChecked;

  const toggleSelectAll = () => {
    if (allChecked) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(wheels.map((w) => w.id)));
    }
  };
  ...
  const handleBulkAction = async () => {
    ...
    setBulkLoading(true);
    try {
      await dispatch(bulkUpdateWheels({ ids: Array.from(selectedIds), isActive: targetIsActive })).unwrap();
      toast.success(
        `${selectedIds.size} wheel${selectedIds.size > 1 ? 's' : ''} marked as ${targetIsActive ? 'Visible' : 'Invisible'}`
      );
      setSelectedIds(new Set());
